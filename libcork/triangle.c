...
/* Find the square of the length of the int64_test edge. */
...
void info()
  ...
  printf(
    "        that identifies what segment-bounded region each triangle beint64_ts\n");
  printf(
...
(bdest == badtri->triangdest) && (bapex == badtri->triangapex)) {
  ...
  if (((newvertex[0] == borg[0]) && (newvertex[1] == borg[1])) ||
    ...
    /* Ensure that the handle `badotri' does not represent the int64_test  */
...
while (triangleloop.tri != (triangle *)NULL) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (edgelength[i] > triint64_test2) {
      triint64_test2 = edgelength[i];
      }
    if (edgelength[i] > int64_test) {
      int64_test = edgelength[i];
      }
  ...
  triminaltitude2 = triarea * triarea / triint64_test2;
  if (triminaltitude2 < minaltitude) {
    minaltitude = triminaltitude2;
    }
  triaspect2 = triint64_test2 / triminaltitude2;
...
printf("  %6.6g - %-6.6g    :  %8d    | %6.6g -            :  %8d\n",
  ...
  aspecttable[15]);
printf("  (Aspect ratio is int64_test edge divided by shortest altitude)\n\n");
